...
impl Config {
    /// Load config from process environment variables. All configuration
    /// is set via the MCP client's `env` block in `.mcp.json`.
    pub fn load() -> Self {
        Self::from_env_map(std::env::vars())
    }

    /// Same as `load`, but uses a caller-supplied env map. Lets tests exercise
    /// the config builder without mutating process-global env state.
